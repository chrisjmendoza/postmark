...
> second batch. #10 (flowOn) closed July 16 2026 — root-caused (SimpleDateFormat race)
> and re-landed as performance-analysis Tier 1 #2, selection verified on device.
> Still open in 🟡: #13 (encryption — needs a passphrase-UX decision),
> #14 (release signing — needs owner-created keystore/secrets; note CI now ships a
> minified debug-keystore `staging` build as of July 16), #20 (Play Store
> workstream — external).
>
...
>
> **July 18 batch (branch `fix/fable-round4`, see `docs/fable-round4.md`):** #25, #28
> (local half), and #31 closed, plus three *unnumbered* end-user findings: notification
> tap now deep-links to the thread (and a latent shared-requestCode PendingIntent
> collision was fixed with it), RCS fallback is disclosed in onboarding/Settings, and
> reactions-are-local gets a one-time notice. New: conversations-list multi-select
> (mark read/unread, bulk pin/mute, provider-backed delete behind confirm +
> default-SMS gate). Pinch-to-zoom text was discovered to have never worked (scroll
> consumed the gesture first) and was re-implemented. All unit tests green; the new
> UI surfaces need on-device verification. Still open here: #13, #14, #20, #30.
...
- [x] 10. Add `.flowOn(Dispatchers.Default)` to `ThreadViewModel`'s render-state combine. *Attempted July 12 and REVERTED the same day: with it in place, on-device message selection stopped responding (selection mode entered, but bubble taps never applied).* **Resolved July 16, 2026** as performance-analysis.md Tier 1 #2: root cause was a shared `SimpleDateFormat` race (fixed July 15 via immutable `DateTimeFormatter`); the re-attempt moved the heavy work into a dedicated `renderPayload` flow on Default while the combine itself stays on Main so selection taps apply synchronously. Selection verified on-device July 16.
...
- [x] 22. Delete `ui/theme/Theme.kt`'s `PostmarkColors`/`LocalPostmarkColors` system (zero consumers) or actually wire it in — bubbles currently don't use the documented `#378ADD` accent. *(Resolved 2026-07-18 on feat/customization: deleted at Chris's direction — the customization feature set superseded the original wire-it-in plan.)*
...
- [x] 25. Add a one-time coach mark for swipe-to-reply / long-press-for-reactions / pinch-to-zoom-text — currently zero in-app discovery path for the app's best gestures. *Done July 18 (`fix/fable-round4`) — one-time dismissible tips card above the thread composer (gated on the thread having messages), a conversations-list hint for the new long-press multi-select, and a first-reaction "reactions stay on your phone" notice (the end-user review's "reactions go nowhere" dealbreaker, disclosed honestly), all backed by a shared `GestureHintsRepository`. Pinch-to-zoom itself was fixed the same day first — it had never actually worked (the list's scroll consumed the gesture before `detectTransformGestures` saw it; re-done via the image viewer's Initial-pass pointer-count arbitration), so the coach mark now advertises a real gesture. Needs on-device verification.*
...
- [x] 28. Clean up the ten already-merged local/remote branches; ~~convert CLAUDE.md to UTF-8~~. *CLAUDE.md converted July 11; 11 merged local branches deleted July 18. Remote deletion was blocked by the shell-permission classifier — the ready-to-run `git push origin --delete …` command is in `docs/fable-round4.md` for the owner.*
...
### 🔵 Housekeeping
- [x] 31. Standardize Toast vs. Snackbar usage (currently 4/4 split for near-identical actions); ~~add haptic feedback (currently zero anywhere in `ui/`)~~ — haptics landed July 16, 2026 (reaction toggle, long-press action-mode entry, row long-press, pin/unpin; performance-analysis Tier 3 #21). *Toast/Snackbar closed July 18 — the settings screens had already converged on the Snackbar `feedback`-flow pattern since this review; only one Toast remained in all of `ui/` (SettingsScreen's "Build info copied"), now converted. Zero Toasts left.*
